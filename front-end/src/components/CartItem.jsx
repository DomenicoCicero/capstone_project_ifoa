...
import { isDeletedFromCart, updateQuantityCartItem } from "../redux/actions";
...
const CartItem = props => {
  ...
  const updateQuantityProductInCartPage = newQuantityProduct => {
    axios
      .post(`/api/cart-item/update/cart-page`, { product_id: props.product.id, quantity: newQuantityProduct })
      .then(data => {
        console.log(data);
  ...
  const handleDecrement = () => {
    if (quantityProduct > 1) {
      ...
      updateQuantityProductInCartPage(quantityProduct - 1);
    }
  };
      ...
      {showAlert && (
        <div className="text-center mt-3">
          <Alert variant="success" className="custom-alert">
            <Alert.Heading>Prodotto eliminato dal carrello</Alert.Heading>
          </Alert>
        </div>
      )}
      {!showAlert && (
